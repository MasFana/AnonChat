...
// Card import removed after hero simplification
...
export default function Home() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const anonId = localStorage.getItem("anonId");
    const res = await fetch(`/api/room/${trimmed}/join`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ anonId }),
    });
        ...
        <main className="relative">
          ...
          <section className="max-w-6xl mx-auto px-4 pt-4 pb-12 md:pb-20">
            <div aria-live="polite" className="min-h-[0]">
              {msg && <div className="mb-6 rounded bg-destructive/10 border border-destructive/40 px-3 py-2 text-destructive animate-fade-in" role="alert">{msg}</div>}
            </div>
            {/* Hero */}
            <div className="max-w-3xl mx-auto space-y-8 text-center">
              <div className="space-y-6">
                <h1 className="text-4xl sm:text-5xl font-extrabold leading-tight tracking-tight">
                  Real‑time Anonymous Chat <span className="block bg-gradient-to-r from-cyan-300 via-sky-400 to-indigo-400 bg-clip-text text-transparent">with Live Polls</span>
                </h1>
                <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                  Spin up a room instantly. Chat live, launch quick polls, then disappear no accounts, no history.
                </p>
                <div className="space-y-4">
                  <div className="mx-auto flex flex-col gap-4 sm:gap-3 sm:flex-row sm:items-stretch max-w-2xl">
                    ...
                    <Button
                      ...
                      disabled={loading}
                      className="relative h-14 px-7 text-lg font-semibold shadow-lg shadow-cyan-500/10 flex-1 sm:flex-none sm:w-auto overflow-hidden group"
                    >
                    ...
                    {/* Divider (visible when horizontal) */}
                    ...
                    >
                      <div className="relative flex-1">
                        <Input
                          ...
                          title="Room ID must be at least 3 characters (letters, numbers, dash, underscore)."
                          className="peer flex-1 h-14 text-base pr-10"
                          disabled={loading}
                          aria-label="Room ID"
                          autoComplete="off"
                        />
                      ...
                      <Button
                        type="submit"
                        disabled={loading || roomId.trim().length < 3}
                        variant="secondary"
                        className="h-14 px-6 font-medium"
                      >Join</Button>
                    </form>
                  </div>
                  {/* Mobile divider */}
                  <div className="sm:hidden flex items-center gap-3 text-[10px] uppercase tracking-wider text-neutral-500">
...
// Lightweight stat helper components (in-file to avoid extra imports for now)
...
function Stat({ number, label }: { number: number; label: string }) {
  return (
    <div className="min-w-[110px] px-4 py-2 rounded-lg border border-border/60 bg-neutral-900/40 backdrop-blur text-center">
      <div className="text-xl font-semibold tabular-nums bg-gradient-to-r from-cyan-300 to-indigo-300 bg-clip-text text-transparent">
        {number}
      </div>
